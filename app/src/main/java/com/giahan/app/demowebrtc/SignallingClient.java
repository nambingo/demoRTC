...
//            });
            ...
            //log event
            socket.on("LOG", args -> Log.e("SignallingClient", "init:  -----> SignallingClient  "+ " log call() called with: args = [" + Arrays.toString(args) + "]"));

            //bye event
            socket.on("bye", args -> callback.onRemoteHangUp((String) args[0]));

            socket.on(Constant.TAG_RTC_CANDIDATE_SOCKET, args -> {
                Log.e("SignallingClient", "init:  -----> VAO DAY DI");
                JSONObject data = (JSONObject) args[0];
                JSONObject sdp;
                try {
                    sdp = data.getJSONObject("sdp");
...
//                    }
    ...
    public void emitMessage(SessionDescription message, Socket socket) {
        try {
            Log.d("SignallingClient", "emitMessage() called with: message = [" + message + "]");
    ...
    public void emitIceCandidate(IceCandidate iceCandidate, Socket socket) {
        try {
...
//            Log.e("SignallingClient", "emitIceCandidate:  -----> "+jsonObject.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
